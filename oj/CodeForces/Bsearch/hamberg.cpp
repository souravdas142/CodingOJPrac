...
class Solution {
    ...
    private:
        ...
        void bsearch() {
            ...
            string bsc_str; cin>>bsc_str;


            ll nb,ns,nc; cin>>nb>>ns>>nc; // available
            ll pb_,ps_,pc_; cin>>pb_>>ps_>>pc_;   // price
            ll r; cin>>r;
            ...
            while(low<=high) {
                ...
                ll ans = low+((high-low)/2);

                ll ansb = max(((b*ans)-nb) *pb_,0);
                ll anss = max(((s*ans)-ns) *ps_,0);
                ll ansc = max(((c*ans)-nc) *pc_,0);

                ll price = (ansb+anss+ansc);

                if(price<=r) low = ans+1;
                else high = ans-1;
